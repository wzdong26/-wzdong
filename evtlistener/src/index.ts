 ...
 */
 export default class EvtListener<EvtName extends string, Evt = any> {
    ...
    private _evtHooksRecord: Record<EvtName, Array<(evt: Evt) => void>> =
        {} as any;
...
export function useEventListener<EvtName extends string, Evt = any>(
    ...
    this.emit = (evtName: EvtName, evt: Evt) =>
        evtListener.emit.bind(evtListener)(evtName, evt, this);
    this.clear = (evtName: EvtName, evt: Evt) =>
        evtListener.clear.bind(evtListener)(evtName);
    return this;
}

// 函数式实现
export function useEvtListener<EvtName extends string, Evt = any>(
    evtNameList?: EvtName[]
) {
    const _evtNameList: EvtName[] | undefined = evtNameList;
    const _evtHooksRecord: Record<
        EvtName,
        Array<(evt: Evt) => void>
    > = {} as any;
    function _findEvtHooks(
        evtName: keyof typeof _evtHooksRecord
    ): Record<EvtName, ((evt: Evt) => void)[]>[EvtName] | never {
        if (_evtNameList && !_evtNameList.includes(evtName))
            throw new Error(`There is no event called ${evtName}!`);
